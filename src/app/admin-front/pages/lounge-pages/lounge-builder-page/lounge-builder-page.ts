...
    }
   
    // 1. Obtener la sesión activa/última sesión por ID de Mesa
    this.tableSessionService.getLatestSessionByTableId(table.diningTableId).subscribe({
      next: (session) => {
        const sessionId = (session as any).publicId; // ID de la sesión para buscar órdenes

        // 2. Obtener las órdenes usando el ID de la Sesión
        this.tableSessionService.getOrdersByTableSession(sessionId, undefined, 0, 100).subscribe({
          next: (orderPage) => {
            this.selectedTableOrders = orderPage.content || [];
          },
          error: (err) => console.error('Error loading orders by session:', err)
        });
      },
      error: (err) => console.error('No active session found for table:', table.diningTableId, err)
    });
  }


  // =======================================================
  // MANEJO DE MODALES Y MODOS
  // =======================================================
...
    }
    // MODO EDICIÓN: selectedTable se usa para abrir TableEditDetailModal
  }
...
    // 1. Finaliza la sesión (esto debería cambiar el estado de la mesa a WAITING_RESET en el backend)
...
    // Usa tu DiningTableService.updateTableStatus
