...
import { FoodVenue } from '../models/food-venue.interface';
...
export class FoodVenueService {
  ...
  getFoodVenues(): Observable<FoodVenue> {
    const cached = this.loadFromCache<FoodVenue>(this.VENUES_CACHE_KEY);
    if (cached) return of(cached);

    return this.http
      .get<FoodVenue>(`${environment.baseUrl}/public/food-venues`)
